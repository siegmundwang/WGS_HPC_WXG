...
QC_fn(){
    ...
    echo "QC_fn called"
    echo "${RESULT}/QC/${1}/${1}_fastp.json"
    echo "out_${1}_[12].fq.gz"
}
...
for i in $(cat ${ALLOC});do
	...
	if [ `grep -c "${i}" "${FINISHED}"` -ne '0' ]; then # 总体判断
        ...
        # 判断是否成功上传
        aws s3api head-object --bucket jiguang2021 --key DNA_RESULT/${1}/score.txt --endpoint-url=http://tos-s3-cn-qingdao.ivolces.com
